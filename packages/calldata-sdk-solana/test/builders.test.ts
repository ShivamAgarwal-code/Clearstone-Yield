...
import {
  ...
  buildWrapperSellPt,
  buildWrapperSellYt,
} from "../src/fixed-yield/builders.js";
...
test("buildWrapperStrip: BN and bigint produce identical data", () => {
  const p = stripParams();
  const a = buildWrapperStrip({ ...p, amountBase: 1_000_000n });
...
test("buildWrapperMerge: discriminator + u64 LE amountPy", () => {
  ...
  for (let i = 11; i < 16; i++) assert.equal(ix.data[i], 0);
});
...
test("buildWrapperMerge: yield_position is writable (pre-/post-maturity both touch it)", () => {
  ...
  const positionSlot = ix.keys.findIndex(
    (k) => k.pubkey.toBase58() === p.yieldPosition.toBase58()
  );
...
test("buildWrapperBuyYt: strip-cascade contributes vault-level accounts", () => {
  ...
  for (const pk of [
    ...
    assert.ok(
      allKeys.has(pk.toBase58()),
      `buy_yt must reference strip-cascade key ${pk.toBase58()}`
    );
  }
});
...
test("buildWrapperSellYt: merge-cascade accounts present, user is sole signer", () => {
  ...
  for (const pk of [
    ...
    assert.ok(
      allKeys.has(pk.toBase58()),
      `sell_yt must reference merge-cascade key ${pk.toBase58()}`
    );
  }
});
