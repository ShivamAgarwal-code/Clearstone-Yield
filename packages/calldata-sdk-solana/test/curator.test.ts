...
import {
  buildCuratorDeposit,
  buildCuratorWithdraw,
  curatorVaultPda,
  curatorBaseEscrowPda,
  curatorUserPositionPda,
} from "../src/fixed-yield/curator.js";
...
test("curatorVaultPda: deterministic, depends on both (curator, baseMint)", () => {
  ...
  const otherCurator = new PublicKey(
    "Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB"
  );
  assert.notEqual(
    a.toBase58(),
    curatorVaultPda(otherCurator, BASE_MINT).toBase58()
  );
});

test("curatorBaseEscrowPda: deterministic and distinct per vault", () => {
  const vault1 = curatorVaultPda(CURATOR, BASE_MINT);
  const vault2 = new PublicKey(
    "Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB"
  );
  assert.equal(
    curatorBaseEscrowPda(vault1).toBase58(),
    curatorBaseEscrowPda(vault1).toBase58()
  );
  assert.notEqual(
    curatorBaseEscrowPda(vault1).toBase58(),
    curatorBaseEscrowPda(vault2).toBase58()
  );
});
...
test("curatorUserPositionPda: keyed by (vault, owner); distinct owners → distinct PDAs", () => {
  ...
  assert.equal(
    curatorUserPositionPda(vault, USER).toBase58(),
    curatorUserPositionPda(vault, USER).toBase58()
  );
  assert.notEqual(
    curatorUserPositionPda(vault, USER).toBase58(),
    curatorUserPositionPda(vault, u2).toBase58()
  );
});
...
test("PDAs honor a programId override", () => {
  ...
  assert.notEqual(
    curatorVaultPda(CURATOR, BASE_MINT).toBase58(),
    curatorVaultPda(CURATOR, BASE_MINT, otherProgram).toBase58()
  );
  const vault = curatorVaultPda(CURATOR, BASE_MINT);
  assert.notEqual(
    curatorBaseEscrowPda(vault).toBase58(),
    curatorBaseEscrowPda(vault, otherProgram).toBase58()
  );
  assert.notEqual(
    curatorUserPositionPda(vault, USER).toBase58(),
    curatorUserPositionPda(vault, USER, otherProgram).toBase58()
  );
});
...
test("buildCuratorDeposit: bigint, BN, and number amounts encode identically", () => {
  ...
  const fromBig = buildCuratorDeposit({ ...p, amountBase: 1_000_000n });
...
test("buildCuratorWithdraw: data = disc + u64 LE shares", () => {
  ...
  assert.deepEqual(
    [...ix.data.subarray(0, 8)],
    [...CURATOR_DISC.withdraw]
  );
  assert.equal(ix.data[8], 0xbb);
  assert.equal(ix.data[9], 0xaa);
  for (let i = 10; i < 16; i++) assert.equal(ix.data[i], 0);
});
...
test("buildCuratorWithdraw: owner is Signer but NOT writable (no rent paid)", () => {
  ...
  assert.equal(
    ...
    false,
    "withdraw owner is Signer but not `mut`"
  );
});
...
test("buildCuratorWithdraw: 7-key layout, no System/Rent at tail", () => {
  ...
  const vault = curatorVaultPda(CURATOR, BASE_MINT);
  const baseDst = new PublicKey(
    "5Q544fKrFoe6tsEbD7S8EmxGTJYAKtTVhAW5Q5pge4j2"
  );
