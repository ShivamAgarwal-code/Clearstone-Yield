...
function noopIx(marker: number): TransactionInstruction {
...
test("packV0Tx: with both unitLimit and microLamportsPerCu, ordering is limit then price", () => {
  ...
  const progAt = (i: number): string =>
    tx.message.staticAccountKeys[
      tx.message.compiledInstructions[i].programIdIndex
    ].toBase58();
...
test("packV0Tx: lookupTables passed through to compileToV0Message and dedupe keys", () => {
  ...
  assert.equal(
    tx.message.addressTableLookups[0].accountKey.toBase58(),
    lutKey.toBase58()
  );
  assert.ok(
    !tx.message.staticAccountKeys.some(
      (k) => k.toBase58() === sharedKey.toBase58()
    ),
    "key present in LUT should not also appear in staticAccountKeys"
  );
});
