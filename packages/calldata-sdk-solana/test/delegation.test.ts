...
import {
  ...
  buildCloseDelegation,
  buildCrankRollDelegated,
  rollDelegationPda,
  decodeRollDelegation,
  slippageFloor,
  MAX_DELEGATION_SLIPPAGE_BPS,
  MIN_DELEGATION_TTL_SLOTS,
  MAX_DELEGATION_TTL_SLOTS,
  ROLL_DELEGATION_ACCOUNT_SIZE,
} from "../src/fixed-yield/delegation.js";
...
test("rollDelegationPda: distinct users produce distinct PDAs", () => {
  ...
  assert.notEqual(
    rollDelegationPda(VAULT, USER).toBase58(),
    rollDelegationPda(VAULT, u2).toBase58()
  );
});
...
test("rollDelegationPda: distinct vaults produce distinct PDAs", () => {
  ...
  assert.notEqual(
    rollDelegationPda(VAULT, USER).toBase58(),
    rollDelegationPda(v2, USER).toBase58()
  );
});
...
test("buildCreateDelegation: data prefix matches discriminator", () => {
  ...
  assert.deepEqual(
    ...
    "first 8 bytes must be the create_delegation sha256 discriminator"
  );
});
...
test("buildCreateDelegation: args serialise as u16 LE + u64 LE (max valid vectors)", () => {
  ...
  for (let i = 14; i < 18; i++) assert.equal(ix.data[i], 0x00);
});
...
test("buildCreateDelegation: account order matches spec", () => {
  ...
  assert.equal(
    ...
    rollDelegationPda(VAULT, USER).toBase58(),
    "delegation must be the (vault, user) PDA"
  );
  // system_program
  assert.equal(
...
test("buildCrankRollDelegated: writability matches spec §3.5", () => {
  ...
  for (const i of mustBeWritable) {
    assert.equal(
      ...
      true,
      `account index ${i} (${ix.keys[i].pubkey.toBase58()}) must be writable`
    );
  }
});
...
test("decodeRollDelegation: roundtrips a hand-built buffer", () => {
  ...
  view.setBigUint64(74, 1_000_000n, true);
  // allocations_hash: 0x11..
  for (let i = 0; i < 32; i++) buf[82 + i] = 0x11;
  // created_at_slot = 500_000
  view.setBigUint64(114, 500_000n, true);
  // bump = 255
  buf[122] = 255;
  ...
  assert.equal(decoded.expiresAtSlot, 1_000_000n);
  assert.equal(decoded.createdAtSlot, 500_000n);
...
test("slippageFloor: matches Rust math bit-for-bit", () => {
  ...
  assert.equal(slippageFloor(1_000_000n, 1_000), 900_000n);
