...
export function createLendingMarket(
  owner: PublicKey,
  marketKp: Keypair,
  quoteCurrency: Buffer = Buffer.alloc(32),
): TransactionInstruction {
...
export function u64Value(n: bigint): Buffer {
...
export function pubkeyValue(pk: PublicKey): Buffer {
  return pk.toBuffer();
}
...
export function configBatch(
  ...
  market: PublicKey,
  reserve: PublicKey,
  updates: Array<{ mode: number; value: Buffer }>,
): TransactionInstruction[] {
  return updates.map(({ mode, value }) =>
    updateReserveConfig(owner, market, reserve, mode, value),
  );
}
