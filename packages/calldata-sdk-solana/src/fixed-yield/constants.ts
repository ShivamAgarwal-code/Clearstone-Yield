...
export const ROUTER_DISC = {
  ...
  /** Kamino-adapter variant of wrapper_buy_pt — used when the SY adapter
   * is `kamino_sy_adapter` (cssol stack) rather than the generic
   * exchange-rate one. Different account list (klend reserve + 8
   * real-klend optional accounts) but same (pt_amount, max_base, max_sy_in)
   * arg layout. Disc = sha256("global:wrapper_buy_pt_kamino")[0..8]. */
  wrapperBuyPtKamino: Buffer.from([0xc9, 0xe5, 0x8f, 0xa9, 0x27, 0x5a, 0xca, 0xc5]),
 ...
 *      "wrapper_withdraw_liquidity_classic","wrapper_buy_pt_kamino"])
