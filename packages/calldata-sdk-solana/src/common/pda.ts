...
export function whitelistPda(mint: PublicKey): PublicKey {
...
export function marketAuthorityPda(market: PublicKey): PublicKey {
  const [pda] = PublicKey.findProgramAddressSync(
...
export function obligationPda(market: PublicKey, owner: PublicKey, seed = 0): PublicKey {
  const seedBuf = Buffer.alloc(1);
  seedBuf.writeUInt8(seed);
  const [pda] = PublicKey.findProgramAddressSync(
    [seedBuf, Buffer.from("obligation"), market.toBuffer(), owner.toBuffer()],
    KLEND_PROGRAM_ID,
  );
  return pda;
}
...
export function reserveCollateralSupplyPda(reserve: PublicKey, market: PublicKey): PublicKey {
  const [pda] = PublicKey.findProgramAddressSync(
...
export function reserveLiquiditySupplyPda(reserve: PublicKey, market: PublicKey): PublicKey {
  const [pda] = PublicKey.findProgramAddressSync(
...
export function reserveFeeVaultPda(reserve: PublicKey, market: PublicKey): PublicKey {
  const [pda] = PublicKey.findProgramAddressSync(
...
export function reserveCollateralMintPda(reserve: PublicKey, market: PublicKey): PublicKey {
  const [pda] = PublicKey.findProgramAddressSync(
