...
import { initializeMint, addToWhitelist, mintTokens, } from "../src/admin/mint.js";
import { createLendingMarket, initReserve, updateReserveConfig, u64Value, pubkeyValue, configBatch, } from "../src/admin/market.js";
import { DELTA_MINT_PROGRAM_ID, KLEND_PROGRAM_ID, KLEND_GLOBAL_CONFIG, TOKEN_2022_PROGRAM_ID, DISC, CONFIG_MODE, } from "../src/common/constants.js";
import { whitelistPda, marketAuthorityPda, ata, reserveCollateralMintPda, reserveLiquiditySupplyPda, reserveCollateralSupplyPda, reserveFeeVaultPda, } from "../src/common/pda.js";
...
test("createLendingMarket: data = disc + 32-byte quoteCurrency (zeroed by default)", () => {
    ...
    assert.equal(ix.keys[2].pubkey.toBase58(), marketAuthorityPda(kp.publicKey).toBase58());
...
test("initReserve: 14 accounts with resolved PDAs (supply, fee, cMint)", () => {
    ...
    assert.equal(ix.keys[5].pubkey.toBase58(), reserveLiquiditySupplyPda(r, market).toBase58());
    assert.equal(ix.keys[6].pubkey.toBase58(), reserveFeeVaultPda(r, market).toBase58());
    assert.equal(ix.keys[7].pubkey.toBase58(), reserveCollateralMintPda(r, market).toBase58());
    assert.equal(ix.keys[8].pubkey.toBase58(), reserveCollateralSupplyPda(r, market).toBase58());
...
test("updateReserveConfig: Borsh = disc + u32 mode + u32 len + value + bool skipValidation", () => {
    ...
    const value = u64Value(7500n); // 75% LTV in bps
    const ix = updateReserveConfig(AUTH, market, reserve, CONFIG_MODE.UpdateLoanToValuePct, value, 
    ...
    // mode u32 LE
    assert.equal(ix.data.readUInt32LE(8), CONFIG_MODE.UpdateLoanToValuePct);
    // vec len u32 LE
    assert.equal(ix.data.readUInt32LE(12), 8);
    // value
    assert.equal(ix.data.readBigUInt64LE(16), 7500n);
    // bool
...
test("updateReserveConfig: defaults skipValidation to true (for batching)", () => {
    const market = WALLET;
    const reserve = new PublicKey("5Q544fKrFoe6tsEbD7S8EmxGTJYAKtTVhAW5Q5pge4j1");
    const ix = updateReserveConfig(AUTH, market, reserve, CONFIG_MODE.UpdateBorrowLimit, u64Value(1000000n));
...
test("configBatch: one ix per update, all scoped to the same (market, reserve)", () => {
    ...
    const reserve = new PublicKey("5Q544fKrFoe6tsEbD7S8EmxGTJYAKtTVhAW5Q5pge4j1");
    const updates = [
        { mode: CONFIG_MODE.UpdateLoanToValuePct, value: u64Value(7500n) },
        { mode: CONFIG_MODE.UpdateBorrowLimit, value: u64Value(1000000n) },
        { mode: CONFIG_MODE.UpdateDepositLimit, value: u64Value(2000000n) },
    ];
