...
import { buildZapInToPt, buildZapOutToBase, } from "../src/fixed-yield/zap.js";
...
test("buildZapInToPt: without sellYt → returns [wrapper_strip] only", () => {
    ...
    assert.deepEqual([...ixs[0].data.subarray(0, 8)], [...ROUTER_DISC.wrapperStrip]);
    assert.equal(ixs[0].programId.toBase58(), CLEARSTONE_ROUTER_PROGRAM_ID.toBase58());
});
// ---------------------------------------------------------------------------
...
test("buildZapInToPt: with sellYt → returns [strip, sell_yt] in order", () => {
    ...
    assert.deepEqual([...ixs[0].data.subarray(0, 8)], [...ROUTER_DISC.wrapperStrip]);
    assert.deepEqual([...ixs[1].data.subarray(0, 8)], [...ROUTER_DISC.wrapperSellYt]);
});
...
test("buildZapInToPt: strip's baseSrc is reused as sell_yt's baseDst (redeem destination)", () => {
    ...
    assert.ok(keys.includes(base.baseSrc.toBase58()), "sell_yt's baseDst must equal strip's baseSrc");
});
