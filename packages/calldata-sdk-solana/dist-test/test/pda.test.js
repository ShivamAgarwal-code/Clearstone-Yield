...
import { vaultAuthorityPda, mintPtPda, mintYtPda, escrowYtPda, yieldPositionPda, marketPda, mintLpPda, marketEscrowPtPda, marketEscrowSyPda, coreEventAuthorityPda, deriveVaultPdas, } from "../src/fixed-yield/pda.js";
...
test("vaultAuthorityPda: deterministic per vault, distinct across vaults", () => {
    assert.equal(vaultAuthorityPda(VAULT_A).toBase58(), vaultAuthorityPda(VAULT_A).toBase58());
    assert.notEqual(vaultAuthorityPda(VAULT_A).toBase58(), vaultAuthorityPda(VAULT_B).toBase58());
});
...
test("yieldPositionPda: distinct per (vault, holder) pair", () => {
    ...
    const otherHolder = new PublicKey("Es9vMFrzaCERmJfrF4H2FYD4KCoNkY11McCe8BenwNYB");
...
test("marketPda: seedId is masked to one byte (u8 semantics)", () => {
    ...
    assert.equal(marketPda(VAULT_A, 0).toBase58(), marketPda(VAULT_A, 256).toBase58());
});
...
test("coreEventAuthorityPda: deterministic (no args)", () => {
    assert.equal(coreEventAuthorityPda().toBase58(), coreEventAuthorityPda().toBase58());
});
test("deriveVaultPdas: bundles per-vault derivations consistently", () => {
    const bundle = deriveVaultPdas(VAULT_A);
    assert.equal(bundle.authority.toBase58(), vaultAuthorityPda(VAULT_A).toBase58());
    ...
    assert.equal(bundle.coreEventAuthority.toBase58(), coreEventAuthorityPda().toBase58());
    // yieldPosition is specifically keyed by (vault, authority) — not (vault, user).
    assert.equal(bundle.yieldPosition.toBase58(), yieldPositionPda(VAULT_A, bundle.authority).toBase58());
});
