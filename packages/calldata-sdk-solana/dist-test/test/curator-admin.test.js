...
import { buildReallocateToMarket, buildReallocateFromMarket, buildMarkToMarket, } from "../src/fixed-yield/curator-admin.js";
import { CURATOR_ADMIN_DISC } from "../src/fixed-yield/constants.js";
import { CLEARSTONE_CURATOR_PROGRAM_ID, CLEARSTONE_CORE_PROGRAM_ID, GENERIC_EXCHANGE_RATE_SY_PROGRAM_ID, TOKEN_PROGRAM_ID, ASSOCIATED_TOKEN_PROGRAM_ID, } from "../src/common/constants.js";
// ---------------------------------------------------------------------------
...
test("buildReallocateToMarket: data prefix matches discriminator", () => {
    ...
    assert.deepEqual([...ix.data.subarray(0, 8)], [...CURATOR_ADMIN_DISC.reallocateToMarket]);
});
...
test("buildReallocateToMarket: trailing programs are the expected defaults", () => {
    ...
    assert.equal(ix.keys[18].pubkey.toBase58(), GENERIC_EXCHANGE_RATE_SY_PROGRAM_ID.toBase58());
    assert.equal(ix.keys[19].pubkey.toBase58(), CLEARSTONE_CORE_PROGRAM_ID.toBase58());
    assert.equal(ix.keys[21].pubkey.toBase58(), ASSOCIATED_TOKEN_PROGRAM_ID.toBase58());
    assert.equal(ix.keys[22].pubkey.toBase58(), SystemProgram.programId.toBase58());
});
...
test("buildReallocateFromMarket: data prefix matches discriminator", () => {
    ...
    assert.deepEqual([...ix.data.subarray(0, 8)], [...CURATOR_ADMIN_DISC.reallocateFromMarket]);
});
...
test("buildReallocateFromMarket: 21-key layout — no ATA+System (no init_if_needed)", () => {
    ...
    assert.equal(ix.keys.length, 21, "reallocate_from_market has no init_if_needed → no ATA / System at tail");
    // Curator is still the only signer.
    const signers = ix.keys.filter((k) => k.isSigner);
    assert.equal(signers.length, 1);
    assert.equal(signers[0].pubkey.toBase58(), common.curator.toBase58());
    // Last key must be core_event_authority (not System).
    assert.equal(ix.keys[20].pubkey.toBase58(), common.coreEventAuthority.toBase58());
});
// ---------------------------------------------------------------------------
...
test("buildMarkToMarket: data = disc + u16 allocationIndex", () => {
    ...
    assert.deepEqual([...ix.data.subarray(0, 8)], [...CURATOR_ADMIN_DISC.markToMarket]);
...
test("buildMarkToMarket: programId defaults to CLEARSTONE_CURATOR_PROGRAM_ID and can be overridden", () => {
    ...
    assert.equal(defaulted.programId.toBase58(), CLEARSTONE_CURATOR_PROGRAM_ID.toBase58());
    const override = dummy(0xfe);
