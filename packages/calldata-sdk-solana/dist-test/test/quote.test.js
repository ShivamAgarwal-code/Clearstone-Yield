...
import { quoteFixedApy, quoteTermDeposit, quoteRequiredDeposit, } from "../src/fixed-yield/quote.js";
...
test("quoteTermDeposit: accepts BN and bigint equivalently", () => {
    ...
    const fromBig = quoteTermDeposit(s, 1000000n);
    const fromBn = quoteTermDeposit(s, new BN(1_000_000));
    assert.equal(fromNum.amountBaseOutAtMaturity.toString(), fromBig.amountBaseOutAtMaturity.toString());
    assert.equal(fromNum.amountBaseOutAtMaturity.toString(), fromBn.amountBaseOutAtMaturity.toString());
});
