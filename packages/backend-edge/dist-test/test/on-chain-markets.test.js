...
import { VAULT_START_TS_OFFSET, VAULT_DURATION_OFFSET, MARKET_PT_BALANCE_OFFSET, MARKET_SY_BALANCE_OFFSET, } from "../src/fixed-yield.js";
// ---------------------------------------------------------------------------
// Buffer builders matching the documented layouts.
// ---------------------------------------------------------------------------
function vaultBuffer(startTs, duration) {
...
function marketBuffer(ptBalance, syBalance) {
    const buf = new Uint8Array(512);
    const v = new DataView(buf.buffer, buf.byteOffset, buf.byteLength);
...
function installConnectionPatch(accounts) {
    ...
    Connection.prototype.getMultipleAccountsInfo = async function (keys) {
        return keys.map((k) => {
            ...
            if (!data)
                return null;
            return {
                data: data, // web3.js types this as Buffer
                executable: false,
                lamports: 0,
...
test("fetchMarketsOnChain: returns decoded markets when registry + RPC both populated", async () => {
    ...
    const accounts = new Map([
        ...
        [registry[0].market, marketBuffer(1000000n, 990000n)], // 0.99
        [registry[1].vault, vaultBuffer(startTs, 90 * 86_400)],
        [registry[1].market, marketBuffer(1000000n, 950000n)], // 0.95
    ]);
    const restore = installConnectionPatch(accounts);
    try {
        const res = await app.fetch(new Request("http://test/fixed-yield/markets"), envWith(registry));
...
test("fetchMarketsOnChain: skips entries whose RPC returns null", async () => {
    ...
    const accounts = new Map([
        ...
        [registry[0].market, marketBuffer(1000000n, 980000n)],
    ]);
    const restore = installConnectionPatch(accounts);
    try {
        const res = await app.fetch(new Request("http://test/fixed-yield/markets"), envWith(registry));
...
test("fetchMarketsOnChain: falls back to fixture when every entry's RPC is missing", async () => {
    const registry = makeRegistry();
    const accounts = new Map(); // every lookup → null
    const restore = installConnectionPatch(accounts);
    try {
        const res = await app.fetch(new Request("http://test/fixed-yield/markets"), envWith(registry));
        const body = (await res.json());
...
test("fetchMarketsOnChain: registry parse error falls through to fixture", async () => {
    ...
    const res = await app.fetch(new Request("http://test/fixed-yield/markets"), badEnv);
    assert.equal(res.status, 200);
    const body = (await res.json());
    assert.ok(body.markets.every((m) => m.id.startsWith("fx-")), "malformed registry must fall through to the fixture");
});
test("fetchMarketsOnChain: orchestration-level index math — infos[i*2] is vault, [i*2+1] is market", async () => {
    ...
    const accounts = new Map([
        ...
        [registry[0].market, marketBuffer(1000000n, 900000n)], // 0.9
        [registry[1].vault, vaultBuffer(1_600_000_000, 10_000)],
        [registry[1].market, marketBuffer(1000000n, 800000n)], // 0.8
    ]);
    const restore = installConnectionPatch(accounts);
    try {
        const res = await app.fetch(new Request("http://test/fixed-yield/markets"), envWith(registry));
...
//# sourceMappingURL=on-chain-markets.test.js.map