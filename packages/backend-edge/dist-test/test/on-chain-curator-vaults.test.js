...
import { CURATOR_VAULT_TOTAL_ASSETS_OFFSET, CURATOR_VAULT_TOTAL_SHARES_OFFSET, CURATOR_VAULT_FEE_BPS_OFFSET, CURATOR_VAULT_ALLOCATIONS_OFFSET, CURATOR_ALLOCATION_SIZE, } from "../src/fixed-yield.js";
// ---------------------------------------------------------------------------
...
test("getCuratorVaults: decodes header + allocations + earliest-maturity nextAutoRollTs", async () => {
    ...
    try {
        const res = await app.fetch(new Request("http://test/fixed-yield/curator-vaults"), envWith(registry));
...
test("getCuratorVaults: dedupes allocation markets across vaults in the second batch", async () => {
    ...
    const mkVault = (deployed) => curatorVaultBuffer({
        curator: CURATOR,
        baseMint: BASE_MINT,
        baseEscrow: BASE_ESCROW,
        totalAssets: 1n,
...
test("getCuratorVaults: CURATOR_VAULT_REGISTRY parse error returns empty list (no throw)", async () => {
    ...
    const res = await app.fetch(new Request("http://test/fixed-yield/curator-vaults"), badEnv);
    assert.equal(res.status, 200);
    const body = (await res.json());
    assert.deepEqual(body.vaults, []);
});
//# sourceMappingURL=on-chain-curator-vaults.test.js.map