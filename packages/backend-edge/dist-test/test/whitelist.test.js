...
function encodeEntry(params) {
    ...
    new DataView(buf.buffer, buf.byteOffset, buf.byteLength).setBigInt64(74, BigInt(params.approvedAt), true);
    buf[82] = 255;
...
test("GET /whitelist: cached path returns KV payload without hitting RPC", async () => {
    ...
    await e.WHITELIST_CACHE.put("all-whitelist", JSON.stringify(payload));
    let rpcCalled = false;
...
test("GET /whitelist: 502 when RPC returns a JSON-RPC error", async () => {
    const mockFetch = async () => new Response(JSON.stringify({ error: { message: "rate-limited" } }), { status: 200, headers: { "content-type": "application/json" } });
    const res = await withFetch(mockFetch, async () => app.fetch(new Request("http://test/whitelist"), env()));
...
test("GET /whitelist/:wallet: filters RPC results to the requested wallet", async () => {
    ...
    const body = await withFetch(mockFetch, async () => {
        const res = await app.fetch(new Request(`http://test/whitelist/${WALLET_A}`), env());
        return (await res.json());
    });
...
//# sourceMappingURL=whitelist.test.js.map