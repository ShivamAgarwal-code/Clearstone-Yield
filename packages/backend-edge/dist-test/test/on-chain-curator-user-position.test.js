...
import { CURATOR_VAULT_TOTAL_ASSETS_OFFSET, CURATOR_VAULT_TOTAL_SHARES_OFFSET, CURATOR_VAULT_FEE_BPS_OFFSET, CURATOR_VAULT_ALLOCATIONS_OFFSET, CURATOR_ALLOCATION_SIZE, CURATOR_USER_POSITION_SHARES_OFFSET, } from "../src/fixed-yield.js";
// ---------------------------------------------------------------------------
// Buffer builders
// ---------------------------------------------------------------------------
...
function marketWithExpiry(expirationTs) {
...
function userPosPda(vault, user) {
...
async function getPosition(vault, user) {
    const res = await app.fetch(new Request(`http://test/fixed-yield/curator-vaults/${vault}/positions/${user}`), ENV);
...
test("fetchCuratorUserPosition: pro-rata math uses bigint, no float truncation at large magnitudes", async () => {
    ...
    const accounts = new Map([
        [userPosPda(VAULT, USER).toBase58(), userPositionBuffer(1000000000n)],
        [
            VAULT,
            curatorVaultBuffer({
                totalAssets: huge,
...
test("fetchCuratorUserPosition: user_pos missing → empty position, no vault decode needed", async () => {
    ...
    accounts.set(VAULT, curatorVaultBuffer({ totalAssets: 1n, totalShares: 1n }));
    const restore = installConnectionPatch(accounts);
    try {
        const body = await getPosition(VAULT, USER);
...
test("fetchCuratorUserPosition: vault has zero totalShares → baseValue 0 (no divide-by-zero)", async () => {
    ...
    const accounts = new Map([
        [userPosPda(VAULT, USER).toBase58(), userPositionBuffer(100n)],
        [VAULT, curatorVaultBuffer({ totalAssets: 1000n, totalShares: 0n })],
    ]);
    const restore = installConnectionPatch(accounts);
    try {
        const body = await getPosition(VAULT, USER);
...
//# sourceMappingURL=on-chain-curator-user-position.test.js.map