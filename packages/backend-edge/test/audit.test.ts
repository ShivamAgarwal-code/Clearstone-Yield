...
function envWithAuditKv() {
  return {
    AUDIT_KV: memKV() as unknown as KVNamespace,
  } as unknown as Env;
}
...
test("POST /audit/log: writes entry + returns { success, key, entry }", async () => {
  ...
  const res = await app.fetch(
    new Request("http://test/audit/log", {
      method: "POST",
  ...
  const body = (await res.json()) as {
    success: boolean;
    key: string;
    entry: { wallet: string; action: string; actor: string; metadata: unknown; timestamp: string };
  };
...
test("POST /audit/log: defaults metadata to {} when omitted", async () => {
  ...
  const body = (await res.json()) as { entry: { metadata: unknown } };
...
test("GET /audit/logs: returns every entry previously logged for the wallet", async () => {
  ...
  const res = await app.fetch(
    new Request("http://test/audit/logs?wallet=w1"),
    env
  );
  assert.equal(res.status, 200);
  const body = (await res.json()) as {
    success: boolean;
    data: Array<{ wallet: string; action: string }>;
    count: number;
  };
...
test("GET /audit/logs: empty list when wallet has no entries", async () => {
  const env = envWithAuditKv();
  const res = await app.fetch(
    new Request("http://test/audit/logs?wallet=never"),
    env
  );
...
test("GET /audit/report/:wallet: groups entries by action count", async () => {
  ...
  const res = await app.fetch(
    new Request("http://test/audit/report/w1"),
    env
  );
  const body = (await res.json()) as {
    data: {
...
test("GET /audit/report/:wallet: empty report for unknown wallet", async () => {
  const env = envWithAuditKv();
  const res = await app.fetch(
    new Request("http://test/audit/report/nobody"),
    env
  );
  const body = (await res.json()) as {
    data: { wallet: string; entries: unknown[]; summary: Record<string, number> };
  };
