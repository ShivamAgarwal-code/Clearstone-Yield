...
function liveDelegation(
  overrides: Partial<LiveDelegation> = {}
): LiveDelegation {
  return {
...
function encodeDelegation(
  ...
  const buf = Buffer.alloc(
    fixedYield.delegation.ROLL_DELEGATION_ACCOUNT_SIZE
  );
...
test("filterLive: nowSlot < expiresAtSlot → kept", () => {
  const live = [liveDelegation({ expiresAtSlot: 1_000n })];
  assert.equal(filterLive(live, 999n).length, 1);
});

test("filterLive: nowSlot == expiresAtSlot → dropped (exact expiry is expired)", () => {
  const live = [liveDelegation({ expiresAtSlot: 1_000n })];
  assert.equal(filterLive(live, 1_000n).length, 0);
});

test("filterLive: nowSlot > expiresAtSlot → dropped", () => {
  const live = [liveDelegation({ expiresAtSlot: 1_000n })];
  assert.equal(filterLive(live, 1_001n).length, 0);
});

test("filterLive: partitions a mixed list", () => {
  const list = [
    liveDelegation({ expiresAtSlot: 900n }),
    liveDelegation({ expiresAtSlot: 1_000n }),
    liveDelegation({ expiresAtSlot: 1_500n }),
  ];
  const kept = filterLive(list, 1_000n);
  assert.equal(kept.length, 1);
  assert.equal(kept[0].expiresAtSlot, 1_500n);
});
...
function mockConnection(
  accounts: Array<{ pubkey: PublicKey; data: Buffer }>
): Connection {
...
test("scanDelegations: groups multiple delegations by vault", async () => {
  ...
  const accounts = [
    {
      pubkey: pda(1),
      data: encodeDelegation(vaultA, userA, 50, 1_000n, 900n),
    },
    {
      pubkey: pda(2),
      data: encodeDelegation(vaultA, userB, 100, 2_000n, 1_500n),
    },
    {
      pubkey: pda(3),
      data: encodeDelegation(vaultB, userC, 25, 3_000n, 2_500n),
    },
  ];
...
test("scanDelegations: pda is carried through from getProgramAccounts result", async () => {
  ...
  const expectedPda = new PublicKey(
    "DoPdypbndDDtjhxQ1P3WozCmHw4rWSF5bzABeBX2VToA"
  );
  const byVault = await scanDelegations(
    mockConnection([
      {
        pubkey: expectedPda,
        data: encodeDelegation(vault, user, 50, 1_000n, 900n),
      },
    ])
  );
...
test("scanDelegations: silently skips accounts that fail to decode", async () => {
  const vault = new PublicKey("EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v");
  const user = new PublicKey("5Q544fKrFoe6tsEbD7S8EmxGTJYAKtTVhAW5Q5pge4j1");
  const good = encodeDelegation(vault, user, 50, 1_000n, 900n);
