...
// Capture harness.
//
// We want to inspect the compiled MessageV0 (ix count, program ids,
// signer metadata, account-meta wiring) without actually serializing.
// The noop sign() skips signing — serialize itself works now that the
// execute paths pass both markets' LUTs into compileToV0Message.
// ---------------------------------------------------------------------------

interface CapturedTx {
  message: MessageV0;
  /** The LUTs passed into compileToV0Message — needed to resolve
   *  account indexes in instructions back to concrete pubkeys. */
  luts: AddressLookupTableAccount[];
}

function installCapture(): { captured: CapturedTx[]; restore: () => void } {
...
function makeMarketFixture(seedBase: number): MarketFixtureKeys {
  return {
...
function encodeMarketTwo(f: MarketFixtureKeys): Buffer {
 ...
 * per-market keys.
 */
function accountsAndLutsFor(fixtures: MarketFixtureKeys[]): {
  accounts: Map<string, Buffer>;
  lutAddresses: Map<string, PublicKey[]>;
} {
  const accounts = new Map<string, Buffer>();
  const lutAddresses = new Map<string, PublicKey[]>();
 ...
 */
function lutAddressesFor(f: MarketFixtureKeys): PublicKey[] {
  return [
    f.mintPt,
    f.mintSy,
    f.mintLp,
...
function vaultSnapshot(
  overrides: Partial<CuratorVaultSnapshot> = {}
...
function makeConfig(curatorKp: Keypair, dryRun = true): KeeperConfig {
  return {
    rpcUrl: "http://mock",
    edgeUrl: "http://mock",
...
interface TxShape {
  ...
  programIds: string[];
  /** Full resolved account-metas per ix (keyed by pubkey base58). */
  ixAccountSets: Array<Set<string>>;
  /** Payer pubkey — must equal the configured signer. */
  payer: string;
}

function describeMessage(cap: CapturedTx): TxShape {
  ...
  const ixAccountSets: Array<Set<string>> = [];
  const programIds: string[] = [];
  for (const ix of msg.compiledInstructions) {
    const progKey = resolved.get(ix.programIdIndex);
    if (!progKey) throw new Error(`unresolved programIdIndex ${ix.programIdIndex}`);
    programIds.push(progKey.toBase58());
    const set = new Set<string>();
    for (const idx of ix.accountKeyIndexes) {
      const k = resolved.get(idx);
      if (!k) throw new Error(`unresolved accountKeyIndex ${idx}`);
      set.add(k.toBase58());
    }
...
test("executeRoll: compiles a 3-ix tx (compute-budget + from + to) with curator as sole signer", async () => {
  ...
  try {
    ...
    assert.equal(
      ...
      3,
      "ComputeBudget + reallocate_from + reallocate_to"
    );
    assert.equal(
      shape.programIds[0],
...
test("executeRoll: account metas on both reallocate ixs reflect the market-header decode", async () => {
  ...
  try {
    ...
    for (const pk of [
      ...
      assert.ok(
        fromAccounts.has(pk.toBase58()),
        `reallocate_from must reference matured-header key ${pk.toBase58()}`
      );
    }
    for (const pk of [
      next.market,
      ...
      assert.ok(
        toAccounts.has(pk.toBase58()),
        `reallocate_to must reference next-header key ${pk.toBase58()}`
      );
    }
  } finally {
    cap.restore();
...
test("executeRoll: throws when a market account is missing from RPC", async () => {
  ...
  await assert.rejects(
    () =>
      executeRoll(conn, makeConfig(curatorKp, true), vault, {
        reason: "ready",
        maturedIndex: 0,
        nextIndex: 1,
        maturedMarket: matured.market.toBase58(),
...
test("executeDelegatedRoll: dryRun returns null without sending", async () => {
  ...
  const cap = installCapture();
  try {
    const sig = await executeDelegatedRoll(
      conn,
      makeConfig(keeperKp, true),
      vault,
      delegation,
      {
        reason: "ready",
  ...
  try {
    await executeDelegatedRoll(
      ...
      delegation,
      {
        ...
        fromMarket: from.market.toBase58(),
        toMarket: to.market.toBase58(),
        deployedBase: 1_000_000_000n,
        minBaseOut: 997_500_000n,
      }
    );
    const shape = describeMessage(cap.captured[0]);
  ...
  try {
    ...
    for (const side of [from, to]) {
      for (const pk of [
        ...
        assert.ok(
          crankAccounts.has(pk.toBase58()),
          `crank ix must reference decoded key ${pk.toBase58()}`
        );
      }
    }
...
test("executeDelegatedRoll: throws when a market account is missing", async () => {
  ...
  await assert.rejects(
    () =>
      executeDelegatedRoll(conn, makeConfig(keeperKp, true), vault, delegation, {
        reason: "ready",
        fromIndex: 0,
        toIndex: 1,
        fromMarket: from.market.toBase58(),
