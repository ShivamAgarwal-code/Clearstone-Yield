...
function vault(overrides: Partial<CuratorVaultSnapshot> = {}): CuratorVaultSnapshot {
  return {
    id: "test-vault",
    label: "USDC Test",
    baseSymbol: "USDC",
...
function liveDelegation(overrides: Partial<LiveDelegation> = {}): LiveDelegation {
  return {
...
test("decideRoll: no-matured-allocation when within grace window", () => {
  const d = decideRoll(
    vault({ nextAutoRollTs: 1_700_000_000 }),
    CURATOR,
    1_700_000_000 + 29, // still inside 30s grace
    30
  );
  assert.equal(d.reason, "no-matured-allocation");
});
...
test("decideDelegatedRoll: delegation-expired when nowSlot >= expiresAtSlot", () => {
  const d = decideDelegatedRoll(
    ...
    1_700_000_100,
    1_000n, // exact expiry — handler treats this as expired
    30
  );
...
test("decideDelegatedRoll: ready computes minBaseOut from slippage floor", () => {
  ...
  const d = decideDelegatedRoll(
    vault({ nextAutoRollTs: 1_700_000_000 }),
    liveDelegation({ maxSlippageBps: 50 }),
    1_700_000_100,
    1_000n,
    30
  );
...
test("decideDelegatedRoll: ready with 0 bps slippage floor == deployedBase", () => {
  const d = decideDelegatedRoll(
    vault({ nextAutoRollTs: 1_700_000_000 }),
    liveDelegation({ maxSlippageBps: 0 }),
    1_700_000_100,
    1_000n,
    30
  );
