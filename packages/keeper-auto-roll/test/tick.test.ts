...
function vaultSnapshot(params: { curator: string; vaultPk: string; nextTs: number }) {
  return {
    id: "vault-1",
    label: "USDC",
    baseSymbol: "USDC",
...
function delegationAccount(vaultPk: string, userPk: string): Uint8Array {
  ...
  v.setBigUint64(74, 999_999_999n, true); // expiresAtSlot — well in the future
...
// ---------------------------------------------------------------------------
// Mock Connection — honours every method runTick / its dependencies touch.
// ---------------------------------------------------------------------------

interface MockConnOpts {
  getSlot?: () => Promise<bigint>;
  delegationAccounts?: Array<{ pubkey: PublicKey; data: Uint8Array }>;
  accountInfo?: Map<string, Uint8Array>;
  lutKeys?: Set<string>;
}

function mockConnection(opts: MockConnOpts = {}): Connection {
  return {
    getSlot: opts.getSlot ?? (async () => 1_000n),
    getProgramAccounts: async () =>
      (opts.delegationAccounts ?? []).map(({ pubkey, data }) => ({
        pubkey,
        account: {
          data,
          executable: false,
...
function installFetch(payload: unknown): () => void {
...
function makeConfig(kp: Keypair, overrides: Partial<KeeperConfig> = {}): KeeperConfig {
  return {
    rpcUrl: "http://mock",
...
function captureLogs<T>(block: () => Promise<T>): Promise<{ result: T; logs: Array<Record<string, unknown>> }> {
...
test("runTick: no delegations + curator-mismatch → tick.skip with reason", async () => {
  ...
  try {
    ...
    const skip = logs.find(
      (l) => l.event === "tick.skip" && l.vault === "vault-1"
    );
...
test("runTick: SKIP_CURATOR_FALLBACK=1 suppresses path 2 when no delegated crank fired", async () => {
  ...
  try {
    ...
    const skip = logs.find(
      (l) => l.event === "tick.skip" && l.reason === "no-delegated-crank"
    );
    assert.ok(skip, "must log the SKIP_CURATOR_FALLBACK branch");
    assert.ok(
      !logs.some((l) => l.event === "auto_roll.completed"),
      "path 2 must not run when SKIP_CURATOR_FALLBACK=1"
    );
  } finally {
    if (prev === undefined) delete process.env.SKIP_CURATOR_FALLBACK;
    else process.env.SKIP_CURATOR_FALLBACK = prev;
    restoreFetch();
  }
});
...
test("runTick: delegation-expired → delegated.skip, no executeDelegatedRoll", async () => {
  ...
  try {
    ...
    // kp != vault.curator).
    assert.ok(
      !logs.some((l) => l.event === "delegated_roll.completed"),
      "expired delegations must not crank"
    );
  } finally {
    restoreFetch();
  }
...
test("runTick: scanDelegations failure logs error but continues to curator-signed path", async () => {
  ...
  try {
    ...
    assert.ok(
      logs.some(
        (l) =>
          l.event === "scan_delegations.error" &&
          String(l.error).includes("RPC exploded")
      ),
      "must log scan_delegations.error"
    );
