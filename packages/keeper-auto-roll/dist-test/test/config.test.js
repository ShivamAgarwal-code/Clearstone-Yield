...
function writeKeypairFixture(dir) {
    const kp = Keypair.generate();
    const path = join(dir, "keypair.json");
    writeFileSync(path, JSON.stringify(Array.from(kp.secretKey)));
    return { path, kp };
}
function withEnv(overrides, block) {
    const original = {};
...
test("loadConfig: throws when CURATOR_KEYPAIR is missing", () => {
    withEnv({ EDGE_URL: "http://edge", CURATOR_KEYPAIR: undefined }, () => {
...
test("loadConfig: defaults applied for optional vars", () => {
    ...
    try {
        ...
        assert.equal(cfg.curatorKeypair.publicKey.toBase58(), kp.publicKey.toBase58());
    }
    finally {
        rmSync(dir, { recursive: true });
    }
...
test("loadConfig: ONE_SHOT / DRY_RUN only trip on '1', not 'true'/'yes'/'TRUE'", () => {
    ...
    try {
        ...
        for (const val of ["true", "yes", "TRUE", "0", "", "on"]) {
            ...
            assert.equal(cfg.oneShot, false, `ONE_SHOT="${val}" must not be truthy`);
            assert.equal(cfg.dryRun, false, `DRY_RUN="${val}" must not be truthy`);
        }
    }
    finally {
        rmSync(dir, { recursive: true });
