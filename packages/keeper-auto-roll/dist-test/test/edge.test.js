...
test("fetchCuratorVaults: issues GET /fixed-yield/curator-vaults", async () => {
    ...
    const mock = async (input) => {
        capturedUrl = typeof input === "string" || input instanceof URL ? input : input.url;
...
test("fetchCuratorVaults: unwraps { vaults: [...] } envelope", async () => {
    ...
    const result = await withFetch(async () => jsonResponse({ vaults: [snapshot] }), () => fetchCuratorVaults("https://edge.test"));
...
test("fetchCuratorVaults: throws on non-2xx with the status code in the message", async () => {
    await withFetch(async () => new Response("boom", { status: 503 }), async () => {
        await assert.rejects(() => fetchCuratorVaults("https://edge.test"), /503/);
    });
});
test("fetchCuratorVaults: tolerates empty vault list", async () => {
    const result = await withFetch(async () => jsonResponse({ vaults: [] }), () => fetchCuratorVaults("https://edge.test"));
...
test("fetchCuratorVaults: respects the caller-supplied base URL", async () => {
    ...
    assert.equal(String(capturedUrl), "https://different.host:8080/api/v2/fixed-yield/curator-vaults");
});
