...
test("contract: fetchCuratorVaults consumes the canonical DTO shape verbatim", async () => {
    const mock = async () => new Response(JSON.stringify({ vaults: [FIXTURE] }), {
        status: 200,
        headers: { "content-type": "application/json" },
    });
    const result = await withFetch(mock, () => fetchCuratorVaults("https://edge.test"));
...
test("contract: decoded snapshot + live delegation feeds decideDelegatedRoll to `ready`", async () => {
    ...
    const d = decideDelegatedRoll(FIXTURE, delegation, FIXTURE.nextAutoRollTs + 60, 1000000n, 30);
    assert.equal(d.reason, "ready");
    if (d.reason === "ready") {
...
test("contract: every field the keeper reads is exercised against the DTO", () => {
    ...
    for (const key of required) {
        assert.ok(key in FIXTURE, `keeper expects ${key} on CuratorVaultSnapshot`);
    }
...
//# sourceMappingURL=contract-edge.test.js.map