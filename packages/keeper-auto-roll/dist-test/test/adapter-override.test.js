...
import { AddressLookupTableAccount, Keypair, PublicKey, TransactionMessage, VersionedTransaction, } from "@solana/web3.js";
...
function marketFixture(seedBase) {
    return {
...
function baseSnapshot(curator, adapter) {
    return {
        id: "v",
        label: "v",
        baseSymbol: "USDC",
...
test("executeRoll: with `adapter` override, snapshot's syMarket + adapterBaseVault appear in the tx", async () => {
    ...
    try {
        ...
        assert.ok(keys.has(kaminoSyMarket.toBase58()), "adapter.syMarket must land in the compiled tx");
        assert.ok(keys.has(kaminoBaseVault.toBase58()), "adapter.adapterBaseVault must land in the compiled tx");
    }
    finally {
        cap.restore();
    }
...
test("executeRoll: without `adapter`, falls back to the generic_exchange_rate_sy seed derivation", async () => {
    ...
    try {
        ...
        const keys = collectKeys(cap.captured[0]);
        assert.ok(!keys.has(kaminoSyMarket.toBase58()), "without override, unrelated pubkey must not land in the tx");
    }
    finally {
        cap.restore();
    }
...
test("executeDelegatedRoll: honours the `adapter` override on the crank ix", async () => {
    ...
    const cap = installCapture();
    try {
        await executeDelegatedRoll(mockConn([from, to]), cfg(kp), snap, delegation, {
            reason: "ready",
            fromIndex: 0,
            toIndex: 1,
...
//# sourceMappingURL=adapter-override.test.js.map