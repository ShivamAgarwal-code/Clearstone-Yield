...
const REFRESH_OBLIGATION_DISC = Buffer.from([33, 132, 147, 228, 151, 192, 72, 89]);
const REQUEST_ELEVATION_GROUP_DISC = Buffer.from([36, 119, 251, 129, 34, 240, 7, 147]);
...
// ---------------------------------------------------------------------------
//
// klend's handler_refresh_obligation expects every reserve referenced by
// the obligation (deposits ∪ borrows) as remaining_accounts (writable).
// Mismatch trips InvalidAccountInput (6006).
...
export function buildRefreshObligationIx(
  ...
  return new TransactionInstruction({
    ...
    keys: [
      ...
      ...obligationReserves.map((r) => ({ pubkey: r, isSigner: false, isWritable: true })),
    ],
    data: REFRESH_OBLIGATION_DISC,
  });
}

// ---------------------------------------------------------------------------
// Instruction: requestElevationGroup
// ---------------------------------------------------------------------------
//
// Stamps the obligation's elevation_group to the requested EG. klend
// validates that:
//   - every deposit reserve allows the new EG (via reserve.elevation_groups)
//   - the obligation's collateral set matches EG.collateral_reserves
//   - max_reserves_as_collateral isn't exceeded
//   - the obligation's debt asset matches EG.debt_reserve (if any)
// All deposit + borrow reserves currently on the obligation must be
// passed as remaining_accounts so klend can run those checks.
//
// Pass elevation_group=0 to drop out of an EG entirely.

export function buildRequestElevationGroupIx(
  owner: PublicKey,
  market: PublicKey,
  obligation: PublicKey,
  ...
  const data = Buffer.concat([
    REQUEST_ELEVATION_GROUP_DISC,
    Buffer.from([elevationGroup & 0xff]),
  ]);
  return new TransactionInstruction({
    programId: KLEND_PROGRAM,
    keys: [
