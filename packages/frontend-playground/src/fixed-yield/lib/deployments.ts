...
/**
 * Live csSOL kyc-kamino stack. Per `clearstone-fixed-yield/orchestrate.md`
 * (2026-05-04 reply): seedId=5 is the first AMM market that produces
 * a valid `core.trade_pt` against the real cssol klend reserve — first
 * trade landed at sig `5XD1ojbZZb…` (9587 SY → 10000 PT, ≈16% APY @ 90d).
 * The earlier seedIds (1–4) are dead state-bloat from the
 * `Number`-tuple-encoding bug that's now fixed; leaving per the doc.
 *
 * `kamino_sy_adapter` upgraded 2026-05-03 to sha `5ac20ecc…`,
 * `clearstone_core` upgraded same day to sha `0cd1f6eb…` (IDL push
 * pending — auth on the IDL PDA is not the cold key).
 */
export const DEVNET_CSSOL_KYC: CanonicalStack = {
  cluster: "devnet",
  rpcUrl: "https://api.devnet.solana.com",
  ...
  kaminoStack: {
    // Underlying csSOL mint (Token-2022).
    baseMint: new PublicKey("6qpu7yCkdKF2D8vnySUNQEQczo5tYGRxbVFfdd8S5Nxt"),
    // SY metadata + mint (KycMode::GovernorWhitelist).
    syMetadata: new PublicKey("7vekBrhxoyV7S4caUP1t32ztrvpoayBHXnWAC5Z7AiDJ"),
    syMint: new PublicKey("jVXmuVB5sEKkq8xYQLfYuggX8QMKVkpT4GSmRsLY3uV"),
    // klend v1 cssol reserve (cToken at `Ej1j2SQL…` is legacy SPL Token).
    klendReserve: new PublicKey("Ez1axBhD6M6t1Zmzfz8MQ95Kmuc48BuoYhQEEHEhT4U1"),
    klendCollateralMint: new PublicKey("Ej1j2SQLjdxY3BEu16tsiEYzXqiMAqbWJ7k83upasTWy"),
    // Accrual oracle OUTPUT account that klend's cssol reserve points
    // at. The accrual_oracle PROGRAM at 8GjxQkJ82LrxpKPYkXw8hpbgCt17hDGk2rcYhqmeR3Ec
    // owns this account; the keeper-cloud Worker calls
    // `accrual_oracle::refresh` on a 5-min cron (rate_bps_per_year=0,
    // so this just mirrors Pyth SOL/USD with a fresh publish_time so
    // klend's `maxAgePriceSeconds=600` doesn't trip on stale prices).
    klendPyth: new PublicKey("3Sx8WJC7y1kokmsu7SoxfJW8nQJktkuQ5fKK8icxPw3P"),
    // pool_escrow / collateral_vault per orchestrate.md table.
    poolEscrow: new PublicKey("HRwvxef9aXEyKSTLkz8jq4xL8MMKvjfiU85Y8Ku8ojwx"),
    collateralVault: new PublicKey("FXQxpuU8VrYmm98vVzqzWb3qnoJsM2L9rXnqNk83bvb1"),
    // Curator vault not enumerated in orchestrate.md — sentinel.
    curatorVault: new PublicKey("11111111111111111111111111111111"),
    // Vault (PT/YT). vaultAuthority is the canonical PDA derived from it;
    // unknown without on-chain read, sentinel for now.
    ptVault: new PublicKey("FWvEaLFdmwRZBAGAcNhrCgFboaETqt2qQVoyusPu12nj"),
    vaultAuthority: new PublicKey("11111111111111111111111111111111"),
    mintPt: new PublicKey("6A9EXzqqnBKdBAbDDNn8Jyja7dRx8FhDMKEC7AwwtbYK"),
    mintYt: new PublicKey("Ga4F1hq27at8m18JVHwHJeRfxyyjbCzeTZQffmDQcFpM"),
    // First trade-able market — seedId=5. Seedids 1..4 are dead.
    ammMarket: new PublicKey("ER2Z72XMzXLugYPXTLPs4mhhtAFN5nFAVaGPR458acHC"),
    // mintLp derived from market on-chain (orchestrate.md table: "derive
    // from market"); sentinel for now — the LP screen reads it lazily.
    mintLp: new PublicKey("11111111111111111111111111111111"),
    marketAlt: new PublicKey("8Jjvdy6ng5b1cisjijW7XkKTkUCA9MvXFgfwghj2tCUr"),
  },
};
...
export const BUILTIN_STACKS: Record<string, CanonicalStack> = {
  // Live cssol kyc-kamino stack — first trade landed 2026-05-04 on
  // seedId=5 market. Default selection.
  "csSOL kyc-kamino (live)": DEVNET_CSSOL_KYC,
  "Solstice USDC (live)": DEVNET_DEFAULT,
  "Test SPL USDC (canonical)": DEVNET_TEST_STACK,
  // csUSDC stack pending — orchestrate.md ask 2 (2026-05-04): no klend
  // reserve over `2VHg4cfQnVGd…` yet. Either kamino tooling stands one
  // up or we adopt an existing devnet USDC reserve.
};

const DEFAULT_ACTIVE_KEY = "csSOL kyc-kamino (live)";
